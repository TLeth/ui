...
class ViewIterator implements Iterator<View> {
  ...
  ViewMatchContext _buildRootCtx() {
    ...
    if (_posOffset > 0) {
      ...
      for (int i = 0; i < _posOffset; i++) {
        ...
        if (!ViewMatchContext.matchType(rt2, seq.type) || !ViewMatchContext.matchClasses(rt2, seq.classes) || //!ComponentLocalProperties.matchAttributes(rt2, seq.attributes) ||
        !new ViewMatchContext(rt2).matchPseudoClasses(seq.pseudoClasses)) return null;

        // check combinator for second and later jumps
        if (i > 0) {
          switch (selector.getCombinator(i - 1)) {
            case SimpleSelectorSequence.COMB_DESCENDANT:
              if (!isDescendant(rt2, rt)) return null;
              break;
            case SimpleSelectorSequence.COMB_CHILD:
              if (rt2.parent != rt) return null;
              break;
            case SimpleSelectorSequence.COMB_GENERAL_SIBLING:
              if (!isGeneralSibling(rt2, rt)) return null;
              break;
            case SimpleSelectorSequence.COMB_ADJACENT_SIBLING:
              if (rt2.previousSibling != rt) return null;
              break;
          }
        }
        rt = rt2;
    ...
    if (_posOffset > 0) for (Selector selector in _selectors) ctx.qualify(selector.selectorIndex, _posOffset - 1); else matchLevel0(ctx);
  ...
  ViewMatchContext _buildNextCtx() {
    ...
    if (_allIds) return null;

    // TODO: how to skip tree branches

    if (_currCtx.view.firstChild != null) return _buildFirstChildCtx(_currCtx);

    while (_currCtx.view.nextSibling == null) {
      _currCtx = _currCtx.parent;
      if (_currCtx == null || _currCtx.view == (_posOffset > 0 ? _offsetRoot : _root)) // TODO: check
  ...
  ViewMatchContext _buildFirstChildCtx(ViewMatchContext parent) {
    ...
    if (_posOffset == 0) matchLevel0(ctx);
    ...
    for (Selector selector in _selectors) {
      ...
      for (int j = posStart; j < selector.seqs.length - 1; j++) {
        switch (selector.getCombinator(j)) {
          case SimpleSelectorSequence.COMB_DESCENDANT:
            if (parent.isQualified(i, j) && checkIDSpace(selector, j + 1, ctx)) ctx.qualify(i, j);
            if (parent.isQualified(i, j) && match(selector, ctx, j + 1)) ctx.qualify(i, j + 1);
            break;
      ...
      for (int j = len - 2; j >= posEnd; j--) {
        ...
        switch (cb) {
          case SimpleSelectorSequence.COMB_DESCENDANT:
            ...
            ctx.qualify(i, j, parentPass && checkIDSpace(selector, j + 1, ctx));
            if (parentPass && match(selector, ctx, j + 1)) ctx.qualify(i, j + 1);
            break;
          case SimpleSelectorSequence.COMB_CHILD:
            ctx.qualify(i, j + 1, parent != null && parent.isQualified(i, j) && match(selector, ctx, j + 1));
            break;
          case SimpleSelectorSequence.COMB_GENERAL_SIBLING:
            if (ctx.isQualified(i, j)) ctx.qualify(i, j + 1, match(selector, ctx, j + 1));
            break;
          case SimpleSelectorSequence.COMB_ADJACENT_SIBLING:
            ctx.qualify(i, j + 1, ctx.isQualified(i, j) && match(selector, ctx, j + 1));
  ...
  static bool isDescendant(View c1, View c2) {
    if (c1 == c2) return true; // first c1 can be IDSpace
    while ((c1 = c1.parent) != null) {
      if (c1 == c2) return true;
      if (c1 is IDSpace) return c1 == c2;
    }
    return false;
  }
  ...
  void matchLevel0(ViewMatchContext ctx) {
    for (Selector selector in _selectors) if (match(selector, ctx, 0)) ctx.qualify(selector.selectorIndex, 0);
  }
  ...
  static int _getCommonSeqLength(List<Selector> list) {
    ...
    for (Selector selector in list) {
      if (strs == null) {
        ...
        for (SimpleSelectorSequence seq in selector.seqs) {
          String id = seq.id;
          if (i >= max || id == null || id.isEmpty || strs[i++] != seq.toString() || strs[i++] != seq.printCombinator()) break;
        }
        if (i-- < max) max = i;
      }
    }
  ...
  static bool _isAllIds(List<Selector> list, int offset) {
    for (Selector s in list) if (s.seqs.length > offset) return false;
    return true;
  }
