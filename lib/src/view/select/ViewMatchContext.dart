...
class ViewMatchContext {
  ...
  ViewMatchContext(this.view)
      : parent = null,
        _qualified = new List<List<bool>>() {
    viewChildIndex = computeViewChildIndex(this.view);
  }

  ViewMatchContext.root(this.view, List<Selector> selectors)
      : parent = null,
        _qualified = _initBoolList(selectors) {
    viewChildIndex = computeViewChildIndex(this.view);
  }

  ViewMatchContext.child(this.view, ViewMatchContext parent)
      : this.parent = parent,
  ...
  bool isQualified(int selectorIndex, int position) {
    if (selectorIndex < 0 || selectorIndex >= _qualified.length) return false;
  ...
  bool isMatched([int selectorIndex = -1]) {
    if (selectorIndex < 0) {
      for (int i = 0; i < _qualified.length; i++) if (isMatched(i)) return true;
      return false;

    } else return selectorIndex < _qualified.length && _qualified[selectorIndex].last;
  }
  ...
  bool match(SimpleSelectorSequence seq) {
    return matchType(this.view, seq.type) //&& matchAttributes(this.view, seq.getAttributes())
    && matchID(this.view, seq.id) //&& matchAttributes(this.view, seq.getAttributes())
    && matchClasses(this.view, seq.classes) //&& matchAttributes(this.view, seq.getAttributes())
  ...
  static bool matchClasses(View view, Iterable<String> classes) {
    ...
    for (String c in classes) if (!view.classes.contains(c)) return false;
    return true;
  }
  ...
  bool matchPseudoClasses(Iterable<PseudoClass> pseudoClasses) {
    ...
    for (PseudoClass pc in pseudoClasses) {
      ...
      if (accept == null) throw new Exception("Pseudo class definition not found: ${pc.name}");
      if (!accept(this, pc.parameter)) return false;
    }
    return true;
  }
  ...
  String toString() {
    ...
    for (List<bool> qs in _qualified) sb.write(qs);
  ...
  static List<List<bool>> _initBoolList(List<Selector> selectors) {
    ...
    for (Selector s in selectors) {
      ...
      for (int i = 0; i < s.seqs.length; i++) sublist.add(false);
    }
    return list;
  }
  ...
  static List<List<bool>> _initBoolListFromParent(ViewMatchContext parent) {
    ...
    for (List<bool> psublist in plist) {
      list.add(sublist = new List<bool>());
      for (int i = 0; i < psublist.length; i++) sublist.add(false);
    }
    return list;
  }
