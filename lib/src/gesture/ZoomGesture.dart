...
class ZoomGestureState extends GestureState {
  final Point _startPos0;
  final Point _startPos1;
  final Point _startMid;
  final Point _startDiff;
  Point _startDir;
  num _scaleBase;
  Point _pos0;
  Point _pos1;
  ...
  ZoomGestureState._(this.gesture, this.eventTarget, Point pos0, Point pos1, int time)
      : _startPos0 = pos0,
        ...
        startTime = time,
        _time = time,
        _startMid = Points.divide(pos0 + pos1, 2),
  ...
  Point get rotation {
    ...
    final num cx = diff.x;
    final num iy = _startDir.y;
    final num ix = _startDir.x;
    final num cy = diff.y;
  ...
  num get angle {
    Point r = rotation;
    return r.x.abs() < r.y.abs() ? (r.y < 0 ? -acos(r.x) : acos(r.x)) : (r.x < 0 ? r.y < 0 ? -PI - asin(r.y) : PI - asin(r.y) : asin(r.y));
  }
  ...
  Transformation get transformation { // TODO: shall supply better assistance
    final Point tr = transition;
    // TODO: shall supply better assistance
    final Point ro = rotation * scalar;
...
class ZoomGesture extends Gesture {
   ...
   * the touch events.
   */
  ZoomGesture(this.owner, {ZoomGestureStart start, ZoomGestureMove move, ZoomGestureEnd end})
      : _start = start,
        _move = move,
        _end = end {
    ...
    _subStart = owner.onTouchStart.listen((TouchEvent event) {
      ...
      final fingers = touches.length;
      if (fingers > 2) {
        _onEnd(event.timeStamp);

      } else if (fingers == 2) {
        final pt0 = touches[0].page;
        final pt1 = touches[1].page;
