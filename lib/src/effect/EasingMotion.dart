...
class EasingMotion extends Motion {
   ...
   */
  EasingMotion(this.action, {EasingFunction easing, int period: 500, int repeat: 1, MotionStart start, MotionEnd end})
      : this.easing = easing,
        this.period = period,
        this.repeat = repeat,
        duration = repeat * period,
        super(start: start, end: end);

  /** Construct an EasingMotion by joining multiple EasingMotion.
   */
  EasingMotion.join(List<EasingMotion> motions, {EasingFunction easing, int period: 500, int repeat: 1, MotionStart start, MotionEnd end}) : this(_jointAction(motions), easing: easing, period: period, repeat: repeat, start: _jointStart(motions, start), end: _jointEnd(motions, end));

  static MotionAction _jointAction(List<EasingMotion> motions) {
    return (num x, MotionState state) {
      for (EasingMotion em in motions) if (identical(em.action(x, state), false)) return false;
    };
  }
  ...
  static MotionStart _jointStart(List<EasingMotion> motions, MotionStart start) {
    return (MotionState state) {
      ...
      for (EasingMotion em in motions) if (em._start != null) (em._start)(state);
    };
  }

  static MotionEnd _jointEnd(List<EasingMotion> motions, MotionEnd end) {
    return (MotionState state) {
      for (EasingMotion em in motions) if (em._end != null) (em._end)(state);
  ...
  num _easingInput(num runningTime) => (duration < 0 || runningTime < duration) ? (runningTime / period) % 1 : 1;

  /** Skip to the end of motion.
   */
  void skip() {
    if (_state != null) doAction_(1, _state.._snapshot(new DateTime.now().millisecondsSinceEpoch));
    super.stop();
  }
