...
class AnchorRelation {
  ...
  AnchorRelation(View view)
    ...
    for (final View child in view.children) {
      ...
      if (av == null) {
        ...
        if (!identical(av.parent, view) && !identical(av, view)) throw new UIError("Anchor can be parent or sibling, not $av");

        final deps = anchored[av];
        if (deps != null) deps.add(child); else anchored[av] = [child];
      }
    }
  }
  ...
  void layoutAnchored(MeasureContext mctx) {
    ...
    for (final View view in indeps) _layoutAnchored(mctx, view);
  }
    ...
    if (views != null && !views.isEmpty) {
      for (final View view in views) {
        if (thisOnly == null || view == thisOnly) {
          ...
          if (thisOnly != null) return; //done
        }
      }

      for (final View view in views) _layoutAnchored(mctx, view, thisOnly); //recursive
    }
  }
}
...
void locateToView(MeasureContext mctx, View view, String location, [View anchor, int x = 0, int y = 0]) {
  ...
  if (anchor != null) {
    final offset = view.style.position == "fixed" ? anchor.page : identical(anchor, view.parent) ? new Point(0, 0) : //parent
    identical(anchor.parent, view.parent) ? new Point(anchor.left, anchor.top) : //sibling (the same coordiante system)
  ...
  if (diff != 0 && (mctx == null || mctx.getWidthByApp(view) == null)) view.width -= diff;
  diff = mi.top + mi.bottom;
  if (diff != 0 && (mctx == null || mctx.getHeightByApp(view) == null)) view.height -= diff;
}
List<int> _getLocators(String loc) {
  ...
  if (locators != null) return locators;
...
const Map<String, List<int>> _locators = const {
  "north start": const [1, 0],
  "north center": const [2, 0],
  "north end": const [3, 0],
  "south start": const [1, 4],
  "south center": const [2, 4],
  "south end": const [3, 4],
  "west start": const [0, 1],
  "west center": const [0, 2],
  "west end": const [0, 3],
  "east start": const [4, 1],
  "east center": const [4, 2],
  "east end": const [4, 3],
  "top left": const [1, 1],
  "top center": const [2, 1],
  "top right": const [3, 1],
  "center left": const [1, 2],
  "center center": const [2, 2],
  "center right": const [3, 2],
  "bottom left": const [1, 3],
  "bottom center": const [2, 3],
  "bottom right": const [3, 3]
};

int _anchorWidth(var anchor, View view) => identical(anchor, view.parent) ? anchor.clientWidth : anchor.offsetWidth;
int _anchorHeight(var anchor, View view) => identical(anchor, view.parent) ? anchor.clientHeight : anchor.offsetHeight;

//TODO: use const to instantiate List when Dart considers Closure as constant
final List<_AnchorLocator> _anchorXLocators = [(int offset, var anchor, View view) { //outer left
...
final List<_AnchorLocator> _anchorYLocators = [(int offset, var anchor, View view) {
